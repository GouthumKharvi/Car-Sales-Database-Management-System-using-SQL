...
SELECT ModelID, COUNT(*) AS CarCount FROM Cars 

GROUP BY ModelID;

# Query20: Find the maximum year of manufacture for each manufacturer
SELECT ManufacturerID, MAX(Year) AS LatestYear FROM Cars
...
SELECT ManufacturerID, COUNT(CarID) AS TotalCars FROM Cars 
...
FROM Car_Sales GROUP BY CarID;

# Query30: List all cars that have not been sold
SELECT * FROM Cars 

WHERE CarID NOT IN (SELECT CarID FROM Car_Sales);
...
SELECT CarID, COUNT(SaleID) AS NumberOfSales

FROM Car_Sales GROUP BY CarID;

# Query33: Find the most expensive car sold in each country
SELECT CountryID, MAX(SalePrice) AS MaxSalePrice 

FROM Car_Sales GROUP BY CountryID;
